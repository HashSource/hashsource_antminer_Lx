...
test_log=/tmp/slttest.log
echo "update start!" >> ${test_log}
...
function spliteFile(){
    ...
    res=$?
    if [ $res -ne 0 ]; then
        echo "BMU CHECK FAILED!" >> ${test_log}
    fi

    echo "spliteFile success!" >> ${test_log}
    return $res
}
...
function writeToNand(){
    ...
    if [ -f minerfs.image.gz ]; then
        echo "write miner fs to nand" >> ${test_log}
...
function splite_slt(){
    echo "splite_slt" >> ${test_log}
    cd /tmp/tmpfw
    local log_path=/tmp/slt_update.log
    local datafile_file=minerfs.image.gz
    local slt_file=ltc.tgz

    local datafile_file_size=$(wc -c "$datafile_file" | awk '{print $1}')

    tail -c 9137140 "$datafile_file" > temp_file
    file_size=9137140
    dd if=temp_file bs=1 of="$slt_file" count=$((file_size))

    echo "datafile_file size: ${datafile_file_size}" >> ${log_path}
    echo "ltc.tgz file size: ${file_size}" >> ${log_path}

    rm temp_file

    cp $slt_file /tmp/ltc.tgz
