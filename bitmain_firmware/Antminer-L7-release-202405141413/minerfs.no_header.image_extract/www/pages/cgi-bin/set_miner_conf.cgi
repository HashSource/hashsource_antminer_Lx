...
function update_config_file()
{
    ...
    #echo "\"bitmain-lowpower-mode\" : ${ant_lowpower_value:-"false"},"  >> $config_file
...
if [[ $? -eq 0 ]]&&[[ ! "$QUERY_STRING_POST"x == ""x ]]; then
    ...
    #ret=`parse_configs $QUERY_STRING_POST '."bitmain-lowpower-mode"'`
    #if [ "$ret"x == "null"x ];then
    #    ant_lowpower_value=`old_configs '."bitmain-lowpower-mode"'`
    #    reload=true
    #else
    #    old_lowpower_value=`old_configs '."bitmain-lowpower-mode"'`
    #    ant_lowpower_value=$ret
    #    if [ $old_lowpower_value != $ret ];then
    #        reload=false
    #    fi
    #fi
