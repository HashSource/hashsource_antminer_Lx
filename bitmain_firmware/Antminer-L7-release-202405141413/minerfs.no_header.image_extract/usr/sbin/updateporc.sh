...
updatebmu_log=/tmp/updatebmu.log
echo "start bmu update!" > ${updatebmu_log}
FILE=$1
if [ -f $FILE ];then
...
/usr/bin/FileParser -f "$subtype" $FILE /etc/bitmain.pub >> ${updatebmu_log}
res=$?
if [ $res -ne 0 ] && [ $res -ne 100 ]; then
    echo $res >> ${updatebmu_log}
    return $res
fi

if [ $res -eq 0 ]; then
    file_path="/usr/bin/compile_time"
    if grep -q "sweep" "$file_path"; then
      echo " $file_path include 'sweep'" >> ${updatebmu_log}
      return $res
    else
      echo " $file_path not include 'sweep'" >> ${updatebmu_log}
    fi
fi
...
set_marker
echo "start slt update 2!" > ${test_log}
return 0
