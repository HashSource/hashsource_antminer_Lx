int __fastcall sub_71BDC(int a1, int a2)
{
  ...
  if ( pthread_mutex_lock((pthread_mutex_t *)(a1 + 28)) )
    sub_6F24C("./../lib/isc/log.c", 959, "((pthread_mutex_lock(((&lctx->lock))) == 0) ? 0 : 34) == 0");
  *(_DWORD *)(a1 + 24) = a2;
  if ( !a2 )
  {
...
LABEL_12:
  ...
  if ( result )
    sub_6F24C("./../lib/isc/log.c", 975, "((pthread_mutex_unlock(((&lctx->lock))) == 0) ? 0 : 34) == 0");
  return result;
}
