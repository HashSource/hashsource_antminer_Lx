double *__fastcall json_vpack_ex(char *a1, int a2, _BYTE *a3, int *a4)
{
  ...
  if ( a3 && *a3 )
  {
    ...
    if ( v8 )
    {
      ...
      if ( (_BYTE)v19 )
      {
        ...
        LOWORD(v12) = 8572;
        LOWORD(v13) = 7748;
        HIWORD(v12) = (unsigned int)"tring" >> 16;
        HIWORD(v13) = (unsigned int)"after reload pool, pool->pool_no = %d, pool->rpc_url = %s" >> 16;
        v8 = 0;
        sub_36DB0(v18, v13, 9, v12);
      }
    }
    return v8;
  }
  ...
  {
    LOWORD(v10) = 7748;
    HIWORD(v10) = (unsigned int)"after reload pool, pool->pool_no = %d, pool->rpc_url = %s" >> 16;
    jsonp_error_init(a1, v10);
    LOWORD(v11) = 8544;
    HIWORD(v11) = (unsigned int)"parse a newline-terminated string" >> 16;
