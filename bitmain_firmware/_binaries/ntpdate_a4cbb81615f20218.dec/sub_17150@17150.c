int __fastcall sub_17150(int a1)
{
  ...
  if ( pthread_mutex_lock((pthread_mutex_t *)(a1 + 28)) )
    sub_15154("./../lib/isc/log.c", 1038, "((pthread_mutex_lock(((&lctx->lock))) == 0) ? 0 : 34) == 0");
...
LABEL_10:
  ...
  if ( result )
    sub_15154("./../lib/isc/log.c", 1048, "((pthread_mutex_unlock(((&lctx->lock))) == 0) ? 0 : 34) == 0");
  return result;
}
