int __fastcall sub_109E0(int a1)
{
  ...
  if ( stream )
  {
    for ( i = 0; ; ++i )
    {
      ...
      fread(v3, *((_DWORD *)&unk_25080 + 33 * i), 1u, stream);
      s = (FILE *)fopen64((char *)&unk_25080 + 132 * i + 4, "wb");
      if ( !s )
        break;
      fwrite(v3, *((_DWORD *)&unk_25080 + 33 * i), 1u, s);
      if ( s )
        fclose(s);
      s = 0;
    }
    printf("Try To Write To File '%s' Failed!\n", (const char *)&unk_25080 + 132 * i + 4);
    if ( stream )
      fclose(stream);
    return 2;
  }
