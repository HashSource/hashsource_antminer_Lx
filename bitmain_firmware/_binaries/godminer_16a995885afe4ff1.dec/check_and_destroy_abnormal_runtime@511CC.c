int check_and_destroy_abnormal_runtime()
{
  ...
  if ( dword_16A718 > 0 )
  {
    ...
    do
    {
      ...
      {
        V_LOCK();
        logfmt_raw(v10, 0x1000u, 0, "chain %d runtime check asic number error, destroy it!", *((_DWORD *)*v2 + 35));
        V_UNLOCK();
        zlog(
          g_zc,
          "/workspace/jenkins/jenkins/workspace/Antminer_L7_release_USE_APW121417b/build/rootfs/buildroot/tmp/release/bui"
          "ld/godminer-origin_master/backend/backend_interface.c",
          163,
  ...
  dword_16A718 = v0;
  return pthread_mutex_unlock(&stru_16A700);
}
