    ...
    if ( total_pools > 0 )
    {
      ...
      do
      {
        while ( 1 )
        {
          ...
          if ( !pool_tget(v4, (unsigned __int8 *)&v4[86].__size[8]) )
          {
            count = v4->__count;
            if ( count < *(_DWORD *)(get_current_pool() + 4) && v21.tv_sec - v4[86].__kind > 120 )
              break;
          }
          if ( total_pools <= v3 )
            goto LABEL_9;
        ...
        V_STR(v24, s, (const char *)v4->__owner);
        v6 = v24[1];
        v7 = v24[2];
        v8 = v24[3];
        v9 = v17;
...
LABEL_9:
    ...
    if ( pool_tget(current_pool, (unsigned __int8 *)&current_pool[86].__size[8]) )
      switch_pools();
    sleep(0x1Eu);
  }
}
